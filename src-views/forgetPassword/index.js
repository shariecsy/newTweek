...
var Root = React.createClass({
	...
	_toSetPassword: function() {
		...
		UAjax.ajax({
			method: 'post',
			url: API_SERVICE_URL + '/v1.0/users/register/checkCode',
			data: {
				mobileNo: mobileNo,
				code: code
			},
			...
			success: function(res) {
				if(res.code == 0) {
					...
					this._layerShow(res.result.msg,function(){
						...
						window.location = '../resetPassword/index.html';
					});
				} else {
		...
		return(
			<div>
				<Header>
					...
					<Header.CenterItem>
						<h1 className="am-header-title">忘记密码</h1>
					</Header.CenterItem>
				</Header>
				<div ref="main">
					<Container>
					...
					<div className="btn-box">
						<button className="btn btn-confirm" ref="btn_next"  onClick={this._toSetPassword}>下一步</button>
					</div>
